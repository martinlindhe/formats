...
func ICO(file *os.File) (*parse.ParsedLayout, error) {
...
func parseICO(file *os.File) (*parse.ParsedLayout, error) {
	...
	for i := 0; i < int(numIcons); i++ {
		...
		resource := parse.Layout{
			...
			Childs: []parse.Layout{
				...
				{Offset: pos + 8, Length: 4, Info: "data size of resource " + resNum, Type: parse.Uint32le},
				{Offset: pos + 12, Length: 4, Info: "offset to resource " + resNum, Type: parse.Uint32le},
			}}
